...
struct ChatView: View {
    ...
    init(userId: String, chatId: String) {
        self._viewModel = StateObject(wrappedValue: { ChatViewModel(userId: userId, chatId: chatId) } ()
        )
        self.userId = userId
        self.chatId = chatId
    }
    ...
    var chatSelection: some View {
        Group {
            if let model = viewModel.chat?.model {
                Text(model.rawValue)
            } else {
                Picker("Model Selection", selection: $viewModel.selectedModel) {
